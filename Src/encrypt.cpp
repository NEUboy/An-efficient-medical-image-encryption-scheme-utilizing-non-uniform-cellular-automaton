...
static void trans_pixs_bit(unsigned char * bitbuff, unsigned char * pix,int num)
{
    for(int n=0;n<num;n++)
    {
        ...
        for(int i=0;i<8;i++)
        {
            bitbuff[n*8+i]=((*(pix+n)&temp)==temp);
            temp=temp<<1;
        }
    }
}

static void trans_bit_pix(unsigned char * bitbuff, unsigned char *pix,int num)
{
    for(int n=0;n<num;n++)
    {
        unsigned char val=0;
...
static void difference_multiply(unsigned char ** sub_pixbuffs[4],int img_size[2])
{
    int sub_height=img_size[0]/2;
    int sub_widght=img_size[1]/2;
    ...
    for(int k=0;k<4;k++)
    {
        for(int i=0;i<sub_height;i++)
        {
            ...
            {
                sub_pixbuffs[(k+1)%4][i][j]=sub_pixbuffs[(k+1)%4][i][j]+sub_pixbuffs[k][i][j];
            }
        }
    }
}
...
static void pix_balance(unsigned char ** bit_buff, double * chaos_buf)
{
    ...
    for(int t=0;t<round;t++)
    {
        ...
        for (int i=1;i<4;i++)
        {
            int divide=((long long)(chaos_buf[order[i]]*1.0e+14)%8);
...
static void subfig_balance(unsigned char ** sub_pixbuffs[4],double ** chaso_x,int img_size[2])
{
    int sub_height=img_size[0]/2;
    int sub_widght=img_size[1]/2;
    ...
    for(int i=0;i<sub_height;i++)
    {
        for(int j=0;j<sub_widght;j++)
        {
            ...
            for(int k=0;k<4;k++)
            {
                ...
                trans_pixs_bit(bit_buff[k],load_pix(sub_pixbuffs[k],k+1,i,j,sub_height,sub_widght),1);
            ...
            for(int k=0;k<4;k++)
            {
                trans_bit_pix(bit_buff[k],load_pix(sub_pixbuffs[k],k+1,i,j,sub_height,sub_widght),1);
                delete [] bit_buff[k];
            }
            delete [] bit_buff;
        }
...
void permutate(image *img,double ** chaos_x)
{
    ...
    //First state in permutation phase
...
void diffusion(image *img,double ** chaos_y)
{
    ...
    {
        bit_buff [k]=new unsigned char [8];
        trans_pixs_bit(bit_buff [k],&pixelbuff[(img_size[0]-1)-(k%img_size[0])][k/img_size[0]],1);
        chaos_buf[k]=chaos_y[(img_size[0]-1)-(k%img_size[0])][k/img_size[0]];
    }
    pix_balance(bit_buff,chaos_buf);
    for(int k=0;k<4;k++)
    {
        trans_bit_pix(bit_buff[k],&pixelbuff[(img_size[0]-1)-(k%img_size[0])][k/img_size[0]],1);
        delete [] bit_buff[k];
    }
    ...
    for(int i=4;i<img_size[0]*img_size[1];i+=4)
    {
        for(int k=0;k<4;k++)
        {
            ...
            bit_buff [k]=new unsigned char [8];
            trans_pixs_bit(bit_buff [k],&pixelbuff[(img_size[0]-1)-((i+k)%img_size[0])][(i+k)/img_size[0]],1);
            chaos_buf[k]=chaos_y[(img_size[0]-1)-((i+k)%img_size[0])][(i+k)/img_size[0]];
        }
        pix_balance(bit_buff,chaos_buf);
        for(int k=0;k<4;k++)
        {
            trans_bit_pix(bit_buff[k],&pixelbuff[(img_size[0]-1)-((i+k)%img_size[0])][(i+k)/img_size[0]],1);
            delete [] bit_buff[k];
        }
    }
